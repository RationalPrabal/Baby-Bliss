...
import NurserySidebar from '@/components/NurserySidebar'
...
let res =  await axios.get(`http://localhost:8080/nursery?_sort=price&_order=${order}`)
...
set_data(res.data)
 ...
 const OrderDiscount=async(order)=>{
  let res =  await axios.get(`http://localhost:8080/nursery?_sort=discount&_order=${order}`)
  console.log(res.data)
  set_data(res.data)
   }
  ...
  const FilterPrice=async(lowerRange,higherRange)=>{
    let res =  await axios.get(`http://localhost:8080/nursery?price_gte=${lowerRange}&price_lte=${higherRange}`)
    console.log(res.data)
    set_data(res.data)
     }
     ...
     const FilterCategory=async(query)=>{
      let res =  await axios.get(`http://localhost:8080/nursery?q=${query}`)
      console.log(res.data)
      set_data(res.data)
       }
  ...
  return (
    ...
    <Box w={{base:"30%", sm:"30%",md:"20%"}}>
      <NurserySidebar OrderPrice={OrderPrice} OrderDiscount={OrderDiscount} FilterPrice={FilterPrice} FilterCategory={FilterCategory}/>
    </Box>
...
export async function getStaticProps(){
    let response = await axios.get("http://localhost:8080/nursery")
    return {
        props:{
