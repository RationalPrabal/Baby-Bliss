...
let res =  await axios.get(`http://localhost:8080/moms?_sort=price&_order=${order}`)
...
set_data(res.data)
 ...
 const OrderDiscount=async(order)=>{
  let res =  await axios.get(`http://localhost:8080/moms?_sort=discount&_order=${order}`)
  console.log(res.data)
  set_data(res.data)
   }
  ...
  const FilterPrice=async(lowerRange,higherRange)=>{
    let res =  await axios.get(`http://localhost:8080/moms?price_gte=${lowerRange}&price_lte=${higherRange}`)
    console.log(res.data)
    set_data(res.data)
     }
     ...
     const FilterCategory=async(query)=>{
      let res =  await axios.get(`http://localhost:8080/moms?q=${query}`)
      console.log(res.data)
      set_data(res.data)
       }
...
export async function getStaticProps(){
    let response = await axios.get("http://localhost:8080/moms")
    return {
        props:{
