...
// function for sort by price
 const OrderPrice=async(order)=>{
  setLoader(true)
let res =  await axios.get(`https://troubled-organized-denim.glitch.me/toys?_sort=price&_order=${order}`)
...
setLoader(false)
 ...
 const OrderDiscount=async(order)=>{
  setLoader(true)
  let res =  await axios.get(`https://troubled-organized-denim.glitch.me/toys?_sort=discount&_order=${order}`)
  console.log(res.data)
  set_data(res.data)
  setLoader(false)
   }
  ...
  const FilterPrice=async(lowerRange,higherRange)=>{
    setLoader(true)
    let res =  await axios.get(`https://troubled-organized-denim.glitch.me/toys?price_gte=${lowerRange}&price_lte=${higherRange}`)
    console.log(res.data)
    set_data(res.data)
    setLoader(false)
     }
     ...
     const FilterCategory=async(query)=>{
      setLoader(true)
      let res =  await axios.get(`https://troubled-organized-denim.glitch.me/toys?q=${query}`)
      console.log(res.data)
      set_data(res.data)
      setLoader(false)
       }
...
export async function getStaticProps(){
    let response = await axios.get("https://troubled-organized-denim.glitch.me/toys")
    return {
        props:{
