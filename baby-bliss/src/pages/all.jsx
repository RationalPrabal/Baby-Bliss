...
const boys = ({ initialData }) => {
  ...
  const OrderPrice = async (order) => {
    ...
    let res = await axios.get(
      `${process.env.NEXT_PUBLIC_baseURL}/all?_sort=price&_order=${order}`
    );
  ...
  const Search = async () => {
    try {
      let res = await axios.get(
        `${process.env.NEXT_PUBLIC_baseURL}/all?q=${text}`
      );
      set_data(res.data);
    } catch (err) {
      console.log(err);
  ...
  const OrderDiscount = async (order) => {
    setLoader(true);
    let res = await axios.get(
      `${process.env.NEXT_PUBLIC_baseURL}/all?_sort=discount&_order=${order}`
    );
  ...
  const FilterPrice = async (lowerRange, higherRange) => {
    setLoader(true);
    let res = await axios.get(
      `${process.env.NEXT_PUBLIC_baseURL}/all?price_gte=${lowerRange}&price_lte=${higherRange}`
    );
  ...
  const FilterCategory = async (query) => {
    setLoader(true);
    let res = await axios.get(
      `${process.env.NEXT_PUBLIC_baseURL}/all?q=${query}`
    );
  ...
  return (
    ...
    >
      ...
      {loader ? (
        <Box w={{ base: "100%", sm: "30%" }} m="auto" mt="50">
